...
def Fix_Special(url):
    dialogprocess.create("PodGod","Renaming paths...",'', 'Please Wait')
...
def Archive_File(sourcefile, destfile):
    ...
    for_progress = []
    ITEM =[]
    dialogprocess.create("PodGod","Archiving...",'', 'Please Wait')
    ...
    for base, dirs, files in os.walk(sourcefile):
        for file in files:
            ...
            if not 'temp' in dirs:
                if not 'plugin.program.podgodwizard' in dirs:
                   import time
                   FORCE= '01/01/1980'
...
def Destroy_Path(path):
    dialogprocess.create("PodGod ","Cleaning...",'', 'Please Wait')
...
def Check_Path():
    if zip=='':
     if dialog.yesno("PodGod Wizard", "You Have Not Set Your Storage Path", 'Set The Storage Path Now ?',''):
        ADDON.openSettings()
        print '######### ZIP DIRECTORY #########'
...
def RestoreIt():
    ...
    dialog = xbmcgui.Dialog()
    if zip == '':
        dialog.ok('PodGod Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    dialogprocess.create("PodGod Wizard","Restoring",'', 'Please Wait')   
    lib=xbmc.translatePath(os.path.join(zip,'backup.zip'))
    Read_Zip(lib)
    dialogprocess.create("PodGod Wizard","Checking ",'', 'Please Wait')
    ...
    dialogprocess.close()
    dialog.ok("PodGod Wizard", "All Done, DONT PRESS OK", "Wait a 5 minutes and pull the Power","")	
def Backupzip():  
    if zip == '':
        dialog.ok('PodGod Wizard','You have not set your ZIP Folder.\nPlease update the addon settings and try again.','','')
        ADDON.openSettings(sys.argv[0])
    ...
    dialogprocess.create("Pod God Wizard","Backing Up",'', 'Please Wait')
    ...
    zipobj.close()
    dialogprocess.close()
    dialog.ok("PodGod Wizard", "You Are Now Backed Up", '','')
