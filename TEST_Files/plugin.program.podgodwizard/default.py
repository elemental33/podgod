...
AddonID = 'plugin.program.podgodwizard'
...
def Wipe_Kodi():
    ...
    if choice == 1:
        if skin!= "skin.confluence":
            Dialog.ok('PodGod','Please Change to the default Confluence skin','before performing a wipe.','')
            xbmc.executebuiltin("ActivateWindow(appearancesettings)")
            return
        else:
            choice = xbmcgui.Dialog().yesno("PodGod", 'This will completely wipe your install.', 'Are you sure?', '', yeslabel='Yes', nolabel='No')
            if choice == 0:
                ...
                exclude_dirs_full =  ['plugin.program.podgodwizard']
            ...
            choice = xbmcgui.Dialog().yesno("Remove PodGod Wizard?", 'Do you also want to remove the PodGod Wizard', 'add-on and have a complete fresh start or would you', 'prefer to keep this on your system?', yeslabel='Remove',nolabel='Keep')
            if choice == 0:
                GoDev.Remove_Textures()
                trpath = xbmc.translatePath(os.path.join(ADDONS,AddonID,''))
                trtemp = xbmc.translatePath(os.path.join(HOME,'..','podgodwizard.zip'))
                GoDev.Archive_File(trpath, trtemp)
                deppath = xbmc.translatePath(os.path.join(ADDONS,'script.module.addon.common',''))
                deptemp = xbmc.translatePath(os.path.join(HOME,'..','podgodwizarddep.zip'))
                ...
                DialogProcess.create("PodGod","Checking ",'', 'Please Wait')
...
def Tools():
    ...
    addFolder('','Convert Links To Special',HOME,'make_special','links.png','','','')
    setView('list', 'MAIN')	
def Clear_Cache():
...
def Remove_Addon_Data():
    choice = xbmcgui.Dialog().yesno('Delete Addon_Data Folder?', 'Free up space by deleting your addon_data folder.', '[COLOR red]***WARNING!***[/COLOR]  This contains all addon related', ' settings including [COLOR red]username[/COLOR] and [COLOR red]password[/COLOR] info.', nolabel='Cancel',yeslabel='Delete')
    if choice == 1:
...
def Restore_Option():
    ...
    setView('list', 'MAIN')
def ComingSoon():
    Dialog.ok('PodGod Wizard','Coming Soon!')			
def ExtraMenu():
    addFolder('folder','PodGod Recommended Addons', 'none', 'ComingSoon', 'podgodbuild.png','','','')
