...
settings = xbmcaddon.Addon( id = 'script.service.LiveScores' )
...
while (not xbmc.abortRequested):
	...
	if go == True:
		if settings.getSetting("enable_service") == 'true':
			...
			search_string2 = settings.getSetting("search_string2")
			search_string2 = word_resolver.searchstr(search_string2)
			if language != 'all':
				search_string2 = search_string2 + '%20lang%3A'+language
			...
			req = urllib2.Request('http://www.twitter.com/search?q='+search_string+search_string2+'&f=realtime'+'&f=tweets')
			...
			try:
				...
				xbmc.log('Live Scores = '+'http://www.twitter.com/search?q='+search_string+'&f=realtime'+'   Number of tweets to skip = '+str(test_tweet))
				...
				if old_text != text:
					xbmc.executebuiltin('XBMC.Notification("%s","%s",%d,"%s")' % ('[B]LiveScores Update[/B]: ', text, display_time, icon))
					xbmc.sleep(wait_time)
					old_text = text
				else:
					xbmc.sleep(5000)
			except:
				xbmc.log('No Live Scores, URL = '+'http://www.twitter.com/search?q='+search_string+'&f=realtime')
				xbmc.sleep(wait_time)			
