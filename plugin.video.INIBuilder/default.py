##############################################################################
#       This file has been modified for INI Builder by podgod (10/15)        #
#               All credit to the original developer Lunatitz                #
##############################################################################
#
...
addon = xbmcaddon.Addon('plugin.video.INIBuilder')
...
DIRS = []
INI_LOC = xbmc.translatePath(addon.getSetting('INI_LOC'))

def addon_log(string):
    # if debug == 'true':
    xbmc.log("[plugin.video.INIBuilder-%s]: %s" %(addon_version, string))
...
def fillPlugins(type='video'):
    ...
    for f in detail:
        ...
        if names and paths:
            ...
            path = paths.group(1)
            if type == 'video' and path.startswith('plugin.video') and not path.startswith('plugin.video.INIBuilder'):
                thumbnail = removeNonAscii(thumbnails.group(1))
                fanart = removeNonAscii(fanarts.group(1))
...
def getSources():
    ...
    addDir('Browse Your Video Addons','video',1,icon,FANART,'[B][COLOR blue]***FOR DEVS ONLY***[/COLOR][/B]. Use this to create addon ini files for TV Guide and those addons based on the original by TV Guide addon by twinther. This is based on original code by Lunatitz. All credit to him.','Add streams to your TV Guide setup from context menu','date','credits')
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
    ...
    contextMenu.append(('[B][COLOR blue]INI Builder[/COLOR][/B] Add Addon ID','XBMC.RunPlugin(%s&mode=201&name=%s)'%(u, name)))
...
def addLink(name,url,mode,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""): 
    ...
    liz.setProperty("Fanart_Image", fanart)
    liz.setProperty('IsPlayable', 'true')
    contextMenu.append(('[B][COLOR blue]INI Builder[/COLOR][/B] Add Channel','XBMC.RunPlugin(%s&mode=200&name=%s)'%(u, name)))
    liz.addContextMenuItems(contextMenu)
...
def makeSTRM(filepath, filename, url):
    addon_log('makeSTRM')
    filepath = os.path.join(INI_LOC, filepath)
...
def writeAddonsINI(url, type=''):
    addon_log('writeAddonsINI')
    thelist = []
    thefile = xbmc.translatePath(os.path.join(profile,'addons.ini'))
    ...
    try:
        ...
        addon_log("writeAddonsINI, Failed " + str(e))
		
          
def writeAddonID(url, type=''):
    addon_log('writeAddonID')
    thelist = []
    thefile = xbmc.translatePath(os.path.join(profile,'addons.ini'))
    theentry = ''.join([type,url])+'\n'
    ...
    try:
        ...
        addon_log("writeAddonID, Failed " + str(e))
		
          
def inAddonsINI(url, type=''):
    addon_log('inAddonsINI')
    # parse settings2 for url return bool if found
    
def removeAddonsINI(url, type=''):       
    addon_log('removeAddonsINI')
    # parse settings2 for url remove entry
    
def getType():
    Types = ['Add This Channel to Addons.ini File' ]
...
def getURL(par):
    try:
        # if par.startswith('?url=plugin://plugin.video.INIBuilder/')
            # url = par.split('?url=')[1]
        # else:
        url = par.split('?url=')[1]
...
if mode==None:
    ...
    addon_log("write ini")
    type = getType()
    writeAddonsINI(url, name)
    fillPluginItems(url, strm=False, strm_name=name, strm_type=type)

elif mode==201:
    writeAddonID('[plugin.video.'+ name + ']')
    # fillPluginItems(url)
